(() => {
  ...
  function renderSource(){const s=state.data.source,node=$('[data-source]');node.classList.toggle('demo',Boolean(state.data.demo));const timestamp=state.checkedAt||s.generated_at;const date=new Date(timestamp);const time=Number.isNaN(date.getTime())?timestamp:new Intl.DateTimeFormat('lt-LT',{dateStyle:'short',timeStyle:'short',timeZone:'Europe/Vilnius'}).format(date);const statusLabel=state.checkedAt?'patikrinta':'duomenys paruošti';node.lastChild.textContent=` ${state.data.demo?'Demonstraciniai duomenys':'Naujausia LEA data: '+s.source_date} · ${statusLabel} ${time}`;if(state.data.demo){const n=$('[data-notice]');n.hidden=false;n.className='notice info';n.textContent='Kol įjungsime kasdienį atnaujinimą, rodoma saugi demonstracinė duomenų kopija.';}}
