(() => {
  ...
  function renderSource(){const s=state.data.source,node=$('[data-source]');node.classList.toggle('demo',Boolean(state.data.demo));const timestamp=state.checkedAt||s.generated_at;const date=new Date(timestamp);const time=Number.isNaN(date.getTime())?timestamp:new Intl.DateTimeFormat('lt-LT',{dateStyle:'short',timeStyle:'short',timeZone:'Europe/Vilnius'}).format(date);const sourceTimestamp=s.source_updated_at||s.source_date;const sourceDate=new Date(sourceTimestamp);const sourceTime=Number.isNaN(sourceDate.getTime())?s.source_date:new Intl.DateTimeFormat('lt-LT',{dateStyle:'short',timeStyle:s.source_updated_at?'short':undefined,timeZone:'Europe/Vilnius'}).format(sourceDate);const statusLabel=state.checkedAt?'patikrinta':'duomenys paruošti';node.lastChild.textContent=` ${state.data.demo?'Demonstraciniai duomenys':'LEA duomenys atnaujinti: '+sourceTime} · ${statusLabel} ${time}`;if(state.data.demo){const n=$('[data-notice]');n.hidden=false;n.className='notice info';n.textContent='Kol įjungsime kasdienį atnaujinimą, rodoma saugi demonstracinė duomenų kopija.';}}
